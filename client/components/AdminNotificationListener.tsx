...
export default function AdminNotificationListener() {
    ...
    try {
      ...
      if (eventType === "booking.updated") {
        ...
        const isPaymentUpdate =
          data.paymentStatus === "completed" || data.paymentStatus === "failed";
        const title = isPaymentUpdate
          ? "💳 Payment Received"
          : "Booking Updated";
        const amount = data.booking?.totalPrice
      ...
      if (
        ...
        const message =
          eventType === "inventory.deleted"
            ? `Item removed: ${data.id}`
            : `Item ${data.item?.id || data.item?.name || ""} updated`;
        toast({ title, description: message, duration: 6000 });
        addAdminNotification({
          type: "inventory",
          title,
      ...
      if (
        ...
        const message =
          eventType === "subscription.renewed"
            ? `Subscription ${data.subscriptionId} renewed`
            : `Subscription ${data.subscriptionId} payment failed`;
        toast({ title, description: message, duration: 6000 });
        addAdminNotification({
          type: "subscription",
          title,
  ...
  const playNotificationSound = (soundType: string) => {
    try {
      ...
      const setting = settings.find(
        (s: any) => s.type === soundType && s.enabled && s.soundEnabled,
      );
      ...
      console.log(
        `🔊 Played notification sound: ${soundType} at volume ${volume}`,
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    subs.push(
      realtimeService.subscribe("booking.created", (d) =>
        handleRealtimeEvent("booking.created", d),
      ),
    );
    subs.push(
      realtimeService.subscribe("booking.updated", (d) =>
        handleRealtimeEvent("booking.updated", d),
      ),
    );
    subs.push(
      realtimeService.subscribe("pos.transaction.created", (d) =>
        handleRealtimeEvent("pos.transaction.created", d),
      ),
    );
    subs.push(
      realtimeService.subscribe("inventory.updated", (d) =>
        handleRealtimeEvent("inventory.updated", d),
      ),
    );
    subs.push(
      realtimeService.subscribe("inventory.created", (d) =>
        handleRealtimeEvent("inventory.created", d),
      ),
    );
    subs.push(
      realtimeService.subscribe("inventory.deleted", (d) =>
        handleRealtimeEvent("inventory.deleted", d),
      ),
    );
    subs.push(
      realtimeService.subscribe("new-message", (d) =>
        handleRealtimeEvent("new-message", d),
      ),
    );
    subs.push(
      realtimeService.subscribe("subscription.renewed", (d) =>
        handleRealtimeEvent("subscription.renewed", d),
      ),
    );
    subs.push(
      realtimeService.subscribe("subscription.failed", (d) =>
        handleRealtimeEvent("subscription.failed", d),
      ),
    );
