...
export class BookingDatabase {
  ...
  updateAnalytics(booking: Booking): void {
    ...
    const analytics =
      this.getAnalytics(today, booking.branch) ||
      this.createEmptyAnalytics(today, booking.branch);
  ...
  private getAnalytics(date: string, branch: string): BookingAnalytics | null {
    ...
    return (
      analytics.find((a) => a.date === date && a.branch === branch) || null
    );
  }
  ...
  private saveAnalytics(analytics: BookingAnalytics): void {
    const allAnalytics = JSON.parse(
      localStorage.getItem("bookingAnalytics") || "[]",
    );
    const existingIndex = allAnalytics.findIndex(
      (a: BookingAnalytics) =>
  ...
  getUserNotifications(userId: string): Notification[] {
    ...
    return notifications.filter(
      (n: Notification) => n.userId === userId || !n.userId,
    );
  }
  ...
  // Image Management
  async saveImage(
    imageData: Omit<ImageAttachment, "id" | "uploadedAt">,
  ...
  updateCrewStatus(
    crewId: string,
    status: User["crewStatus"],
    currentAssignment?: string,
  ): boolean {
    const users = JSON.parse(localStorage.getItem("registeredUsers") || "[]");
    const crewIndex = users.findIndex(
      (u: User) => u.id === crewId && u.role === "crew",
    );
  ...
  addStatusUpdate(
    ...
    updatedBy: string,
    notes?: string,
    location?: BookingStatusUpdate["location"],
  ): BookingStatusUpdate {
  ...
  createSystemNotification(
    notificationData: Omit<SystemNotification, "id" | "createdAt" | "readBy">,
  ): SystemNotification {
...
export const createBooking = (
  data: Omit<Booking, "id" | "createdAt" | "updatedAt" | "confirmationCode">,
...
export const getUserBookings = (userId: string) =>
  bookingDB.getUserBookings(userId);
...
export const saveBookingImage = (
  imageData: Omit<ImageAttachment, "id" | "uploadedAt">,
) => bookingDB.saveImage(imageData);

export const getBookingImages = (
  bookingId: string,
  type?: ImageAttachment["type"],
...
export const assignCrewToBooking = (
  bookingId: string,
  crewIds: string[],
  assignedBy: string,
...
export const updateCrewAssignmentStatus = (
  assignmentId: string,
  status: CrewAssignment["status"],
  notes?: string,
) => bookingDB.updateCrewAssignmentStatus(assignmentId, status, notes);

export const updateCrewStatus = (
  crewId: string,
  status: User["crewStatus"],
  currentAssignment?: string,
...
export const addBookingStatusUpdate = (
  bookingId: string,
  status: Booking["status"],
  updatedBy: string,
  notes?: string,
...
export const getUserSystemNotifications = (
  userId: string,
  userRole: User["role"],
) => bookingDB.getUserSystemNotifications(userId, userRole);

export const markSystemNotificationAsRead = (
  notificationId: string,
  userId: string,
