...
router.post('/register-token', async (req, res) => {
  try {
    ...
    const success = await pushNotificationService.registerToken({
      ...
      deviceName,
      notificationTypes: ['booking_updates', 'loyalty_updates', 'system', 'achievements'],
    });
...
router.post('/unregister-token', async (req, res) => {
  try {
    ...
    const success = await pushNotificationService.unregisterToken(token, userId);
...
router.post('/booking-update', async (req, res) => {
  try {
    ...
    res.json({ 
      success,
      message: success ? 'Booking notification sent' : 'Failed to send booking notification'
    });
  } catch (error) {
...
router.post('/loyalty-update', async (req, res) => {
  try {
    ...
    res.json({ 
      success,
      message: success ? 'Loyalty notification sent' : 'Failed to send loyalty notification'
    });
  } catch (error) {
...
router.post('/achievement', async (req, res) => {
  try {
    ...
    res.json({ 
      success,
      message: success ? 'Achievement notification sent' : 'Failed to send achievement notification'
    });
  } catch (error) {
