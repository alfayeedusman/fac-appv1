...
async function syncVariables(variables) {
  ...
  keys.forEach(key => {
    ...
    const isSensitive = ['TOKEN', 'KEY', 'SECRET', 'PASSWORD', 'URL', 'ID'].some(
  ...
  if (process.argv.includes('--skip-confirm')) {
    ...
    await new Promise(resolve => {
      readline.question(`\n${colors.yellow}Continue syncing to Netlify? (yes/no): ${colors.reset}`, answer => {
        readline.close();
  ...
  // Summary
  log(colors.blue, '\n' + '='.repeat(60));
  log(colors.green, `✅ Successfully synced: ${successCount}/${keys.length} variables`);

  if (failedVars.length > 0) {
    log(colors.red, `❌ Failed to sync: ${failedVars.length} variables`);
    log(colors.yellow, `\nFailed variables: ${failedVars.join(', ')}`);
    log(colors.yellow, '\nTroubleshooting:');
    log(colors.cyan, '  1. Check if netlify-cli is installed: netlify --version');
  ...
  log(colors.cyan, '     https://app.netlify.com → Site settings → Build & deploy → Environment');
  log(colors.cyan, '  2. Commit and push your code:');
  log(colors.cyan, '     git add . && git commit -m "Add env vars sync script"');
  log(colors.cyan, '     git push origin main');
  log(colors.cyan, '  3. Netlify will automatically deploy with these variables\n');
